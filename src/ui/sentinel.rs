...
pub fn render_sentinel_mode(f: &mut Frame, app: &mut App) {
    ...
    let color = if running_status == "RUNNING" {
        Color::Green
    } else {
        Color::Red
    };
    ...
    if let Some(state) = &app.sentinel_state {
        ...
        let status_list = Paragraph::new(status_text).block(
            Block::default()
                .title(" Recent Status ")
                .borders(Borders::ALL),
        );
        ...
        // Interval Setting
        let interval_style =
            if app.active_tab().input_mode == crate::app::InputMode::EditingSentinelInterval {
                Style::default().fg(Color::Yellow)
            } else {
                Style::default()
            };
        ...
        let interval_widget = Paragraph::new(format!("{} s", app.sentinel_interval_input))
            .block(
                Block::default()
                    .title(" Interval (Press 'i') ")
                    .borders(Borders::ALL)
                    .border_style(interval_style),
            )
            .alignment(Alignment::Center);
        ...
        if last_failed {
            ...
            let alert = Paragraph::new(" ⚠️ ALERT ").style(
                Style::default()
                    .bg(Color::Red)
                    .fg(Color::White)
            ...
            let area = f.area();
            let alert_area = ratatui::layout::Rect {
