...
fn create_app_with_env(vars: Vec<(&str, &str)>) -> App {
    ...
    if app.environments.is_empty() {
        app.environments
            .push(crate::domain::environment::Environment {
                name: "Test".to_string(),
...
fn test_no_substitution_needed() {
    let mut app = create_app_with_env(vec![("baseUrl", "https://api.example.com")]);
...
fn test_multiple_occurrences() {
    let mut app = create_app_with_env(vec![("id", "123")]);
...
fn test_partial_match_should_not_substitute() {
    let mut app = create_app_with_env(vec![("base", "BASIC"), ("baseUrl", "FULL")]);
